...
import { changeContactMeActive, changeContactMeClosed, changeContactMeOnTaskbar, changeContactMeMax, changeAllActiveToFalse, incrementGlobalZ } from "../Redux/homeSlice";
...
const ContactMeWindow = () => {
    ...
    return (
        <>
        <div className={isContactMeMax ? `w-screen h-screen fixed top-0 right-0 bottom-0 left-0 z-6`: "hidden"}>
            ...
            <Draggable handle="#handle"  bounds="parent" defaultPosition={{x: 600, y: -200}} onStart={()=>{dispatch(incrementGlobalZ());setActiveWindow()}}>
                <Resizable style={isContactMeClosed ? {zIndex: -100, position: "absolute"} : isContactMeActive ? {zIndex:zValue, position: "absolute"} : {zIndex: 5, position: "absolute"}} defaultSize={{ width: 500, height:275}} minWidth={300} minHeight={200} enable={!isContactMeClosed ? { top:false, right:true, bottom:true, left:false, topRight:false, bottomRight:true, bottomLeft:false, topLeft:false } : { top:false, right:false, bottom:false, left:false, topRight:false, bottomRight:false, bottomLeft:false, topLeft:false }}>
