...
const SkillsWindow = () => {
    ...
    const [divSize, setDivSize] = useState({width: screenSize.width*(1/3), height: screenSize.height*(7/12)})
            ...
            <div className={isSkillsMax ? `w-screen h-screen fixed top-0 right-0 bottom-0 left-0`: "hidden"} style={isSkillsMax ? { zIndex: zValue}:{}}>
                <div className={!isSkillsClosed ? "border-4 divide-y-4 border-taskbar flex flex-col h-full relative" : "hidden"} onClick={()=>setActiveWindow()}>
                    <div id="handle" className="flex justify-between bg-title-bar text-white">
                        ...
                        <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();skillsMinimizeOnClick()}} onTouchStart={(event)=>{event.stopPropagation();skillsMinimizeOnClick()}}><img draggable={false} alt="hidebutton" src={hidebutton}/></button>
                                <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();skillsMaxOnClick()}} onTouchStart={(event)=>{event.stopPropagation();skillsMaxOnClick()}}><img draggable={false} alt="maximizebutton" src={isSkillsMax ? minimizebutton :maximizebutton}/></button>
                                <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();skillsCloseOnClick()}} onTouchStart={(event)=>{event.stopPropagation();skillsCloseOnClick()}}><img draggable={false} alt="closebutton" src={closebutton}/></button>
                        </div>
                    </div>
