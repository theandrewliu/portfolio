            ...
            <div className={isOverRatedMax ? `w-screen h-screen fixed top-0 right-0 bottom-0 left-0`: "hidden"} style={isOverRatedMax ? { zIndex: zValue}:{}}>
                <div className={!isOverRatedClosed ? "border-4 divide-y-4 border-taskbar flex flex-col h-full relative" : "hidden"} onClick={()=>setActiveWindow()}>
                        <div id="handle" className="flex justify-between bg-title-bar text-white">
                            ...
                            <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();OverRatedMinimizeOnClick()}} onTouchStart={(event)=>{event.stopPropagation();OverRatedMinimizeOnClick()}}><img draggable={false} alt="hidebutton" src={hidebutton}/></button>
                                <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();OverRatedMaxOnClick()}} onTouchStart={(event)=>{event.stopPropagation();OverRatedMaxOnClick()}}><img draggable={false} alt="minmaxbutton" src={isOverRatedMax ? minimizebutton :maximizebutton}/></button>
                                <button className="active:border-t-shadow active:border-l-shadow active:border-b-white active:border-r-white bg-taskbar border border-t-white border-l-white border-b-shadow border-r-shadow h-3 w-3 scale-125" onClick={(event)=>{event.stopPropagation();OverRatedCloseOnClick()}} onTouchStart={(event)=>{event.stopPropagation();OverRatedCloseOnClick()}}><img draggable={false} alt="closebutton" src={closebutton}/></button>
                            </div>
                        </div>
